class RecentController < ApplicationController
  ...
  expose (:recently_created_collection_item_comments){Comment.order('created_at DESC').limit(40)}
  expose (:recently_created_page_comments){Comment.order('created_at DESC').limit(40)}
  expose (:recently_created_resource_comments){Comment.order('created_at DESC').limit(40)}
