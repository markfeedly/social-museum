module CheckConflictingChanges
  def changed_collection_item(coll_item)
    ...
    desired_categories  = clean_str( params[:collection_item][:tags_as_str] )
    desired_location    = params[:collection_item][:title_attributes][:title]
  ...
  def clean_str(str)
    str.split(',').collect{|t| t.strip.squeeze(' ')}.sort.uniq.reject{|t|t==''}
  end
end