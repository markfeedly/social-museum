class CategoriesController < ApplicationController
  ...
  expose(:categorised_collection_items) do
    collection = category_is ? CollectionItem.all.select { |ci| ci.in_category_and_its_children?(params[:id]) ? ci : nil } : []
    Kaminari.paginate_array(collection).page(params[:page]).per(10)
  end
  expose(:categorised_pages) do
    collection = category_is ? category_is.pages.select { |ci| ci.in_category_and_its_children?(params[:id]) ? ci : nil } : []
    Kaminari.paginate_array(collection).page(params[:page]).per(10)
  end
  expose(:categorised_resources) do
    collection = category_is ? category_is.resources.select { |ci| ci.in_category_and_its_children?(params[:id]) ? ci : nil } : []
    Kaminari.paginate_array(collection).page(params[:page]).per(10)
  end

  expose(:ci_count) { category_is ? categorised_collection_items.count : 0 }
  expose(:p_count)  { category_is ? categorised_collection_items.count : 0 }
